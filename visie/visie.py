...
def backronyms(acronym, min_length=3, dict_path=DICT_PATH):
    with open(dict_path, 'r') as dictionary:
        ...
        for word in (w.strip() for w in dictionary.readlines()):
            if len(word) < min_length:
                continue
            c = word[0].lower()
            if c in required_letters:
                possibilities[c].append(word)
        return itertools.product(*(possibilities[c.lower()] for c in acronym))
